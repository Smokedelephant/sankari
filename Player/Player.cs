...
public class Player : KinematicBody2D
{
	...
	private GTimer _timerDashDelay;
	...
	public override void _Ready()
	{
		...
		_timerDashDelay = new GTimer(this, nameof(DoDashStuff), 7, false, false);
	...
	private void HandleMovement(float delta)
	{
		...
		{
			// adds extra time between when the player presses dash so the player can change dash directions
			_timerDashDelay.Start();
			HandleDash();
		}
		else
			_gravity = GRAVITY_AIR;
		// apply gravity
		// In the near future someone will need to make gravity stop applying if going up a slope and start when off the slope
